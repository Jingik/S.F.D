...
def preprocess_config(config):
    ...
    max_epochs = config['common'].get('max_epochs', 30)  # 기본값 30
    config['trainer']['params'] = config.get('trainer', {}).get('params', {})
    config['trainer']['params']['max_epochs'] = max_epochs

    # Learning rate scheduler 관련 설정
...
def train_with_cv(config, model, train_val_file_paths, train_val_labels, test_dataloader,
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_val_file_paths, train_val_labels)):
        ...
        # Trainer 설정 (로깅 및 callbacks 추가)
        tensorboard_logger = TensorBoardLogger(save_dir=log_dir, name='tensorboard_logs')
        ...
        trainer = pl.Trainer(
            ...
            callbacks=[getattr(importlib.import_module(callback_config.target.rsplit('.', 1)[0]),
...
if __name__ == '__main__':
    ...
    if config['common']['use_cross_validation']:
        ...
        # Trainer 설정
        tensorboard_logger = TensorBoardLogger(save_dir=config['common']['exp_name'], name='tensorboard_logs')
        csv_logger = CSVLogger(save_dir=config['common']['exp_name'], name='csv_logs')

        trainer = pl.Trainer(
            default_root_dir=config['common']['exp_name'],
